...
pub fn build(b: *std.Build) !void {
    ...
    const cfg_module: std.Build.Module.CreateOptions = .{
    ...
    const sdl3 = b.addModule("sdl3", .{
        .root_source_file = cfg_module.root_source_file,
        .target = target,
        .optimize = optimize,
    });
    ...
    if (ext_image) {
        setupSdlImage(b, sdl3, sdl_dep_lib, cfg_module);
    }
}

// Most of this is copied from https://github.com/allyourcodebase/SDL_image/blob/main/build.zig.
pub fn setupSdlImage(b: *std.Build, sdl3: *std.Build.Module, sdl_dep_lib: *std.Build.Step.Compile, cfg_module: std.Build.Module.CreateOptions) void {
    const upstream = b.lazyDependency("sdl_image", .{}).?;

    const target = cfg_module.target orelse b.standardTargetOptions(.{});
